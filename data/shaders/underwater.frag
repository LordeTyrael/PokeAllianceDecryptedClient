...
vec4 bluescale(vec4 color)
{
	float blue = dot(color.rgb, vec3(0.000, 0.000, 1.00));
	return vec4(color.r * 0.7, color.g * 0.7, blue, 1);
}
...
void main(void)
{
	...
	gl_FragColor = bluescale(texture2D(u_Tex0,c1)*(alpha));
}
