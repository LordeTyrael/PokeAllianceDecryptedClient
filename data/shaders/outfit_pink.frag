...
void main(void)
{
    ...
    if (col.a > 0.0) {
        ...
        if (isBorder) {
            // Efeito de pulsação: varia entre um tom mais escuro e um pink vibrante
            float pulse = 0.5 + 0.5 * sin(u_GlobalTime * 10.0); // Ajuste a velocidade conforme desejar
            vec3 darkColor = vec3(0.1, 0.0, 0.1);    // tom base escuro (pink escuro)
            vec3 pinkColor = vec3(0.92, 0.0, 0.92);    // tom vibrante de pink
            vec3 finalColor = mix(darkColor, pinkColor, pulse);
