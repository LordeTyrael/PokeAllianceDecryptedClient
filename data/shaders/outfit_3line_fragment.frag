...
void main()
{
    ...
    if(texcolor.a > 0.9) {
        ...
        float frame = mod(p.y, height) - mod(u_GlobalTime * speed, height);
        float dist1 = min(abs(frame), min(abs(frame - height), abs(frame + height)));
        frame = mod(p.y + height * 0.33, height) - mod(u_GlobalTime * speed, height);
        float dist2 = min(abs(frame), min(abs(frame - height), abs(frame + height)));
        frame = mod(p.y + height * 0.66, height) - mod(u_GlobalTime * speed, height);
