...
MouseActions = {
    ['LMB'] = MouseLeftButton,
    ['MMB'] = MouseMidButton,
    ['RMB'] = MouseRightButton,
    ['MouseX3'] = MouseTouch,
    ['MouseX1'] = MouseButton4,
    ['MouseX2'] = MouseButton5
}
...
useWith = nil
defaultComboKeys = {
["F1"] = {autoSend = true, value = '#s m1'},
["F2"] = {autoSend = true, value = '#s m2'},
["F3"] = {autoSend = true, value = '#s m3'},
["F4"] = {autoSend = true, value = '#s m4'},
["F5"] = {autoSend = true, value = '#s m5'},
["F6"] = {autoSend = true, value = '#s m6'},
["F7"] = {autoSend = true, value = '#s m7'},
["F8"] = {autoSend = true, value = '#s m8'},
["F9"] = {autoSend = true, value = '#s m9'},
["F10"] = {autoSend = true, value = '#s m10'},
["Ctrl+Up"] = {autoSend = true, value = '#s t1'},
["Ctrl+Right"] = {autoSend = true, value = '#s t2'},
["Ctrl+Down"] = {autoSend = true, value = '#s t3'},
["Ctrl+Left"] = {autoSend = true, value = '#s t4'},
["MMB"] = {autoSend = false, useType = 3, itemId = 3453},
}
perCharacter = true
...
function init()
    if not g_app.isMobile() then
        hotkeysButton = modules.client_topmenu.addMiddleGameToggleButton('hotkeysButton', tr('Hotkeys') .. ' (Ctrl+K)',
            '/images/ui/topbuttons/hotkeys', toggle, false, 7)
    end
...
function load(forceDefaults)
    ...
    count = 0
    for name, config in pairs(hotkeyList) do
...
function unload()
    ...
    for keyCombo, callback in pairs(boundCombosCallback) do
        if MouseActions[keyCombo] then
            g_mouse.unbindPress(keyCombo, gameRootPanel)
        else
...
function loadDefautComboKeys()
    for name, config in pairs(defaultComboKeys) do
        addKeyCombo(name, config)
    end
    save()
    reload()
end
...
function addHotkey()
    ...
    assignWindow.onMouseRelease = hotkeyMCapture
end
...
function addKeyCombo(keyCombo, keySettings, focus)
    ...
    if not hotkeyLabel then
        ...
        if keyCombo:lower():find("ctrl") then
            if MouseActions[keyCombo] then
                g_mouse.unbindPress(keyCombo, gameRootPanel)
            elseif boundCombosCallback[keyCombo] then
        ...
        if MouseActions[keyCombo] then
            boundCombosCallback[keyCombo] = function(k, c, ticks)
                if c == MouseActions[keyCombo] then
                    prepareKeyCombo(keyCombo, 0, k)
                end
            end

            g_mouse.bindPress(gameRootPanel, boundCombosCallback[keyCombo], MouseActions[keyCombo])
        else
            boundCombosCallback[keyCombo] = function(k, c, ticks)
...
function prepareKeyCombo(keyCombo, ticks, k)
    local hotKey = hotkeyList[keyCombo]
    if (keyCombo:lower():find("ctrl") and not hotKey) or
        (hotKey and hotKey.itemId == nil and (not hotKey.value or #hotKey.value == 0) and not hotKey.action) then
...
function doKeyCombo(keyCombo, repeated, pos)
    if not g_game.isOnline() then
        return
    end
    ...
    elseif hotKey.useType == HOTKEY_MANAGER_USEONTARGET then
        ...
        if not attackingCreature then
            ...
            if pos then
                ...
                tileItem = tile:getGround()
                if tileItem then
                    local player = g_game.getLocalPlayer()
                    g_game.useInventoryItemWith(hotKey.itemId, tileItem)
                end
    ...
    if hotkeysWindow.action then
        if currentHotkeyLabel then
            ...
            if currentHotkeyLabel.action then
                hotkeysWindow.action:setCurrentIndex(translateActionToActionComboboxIndex(currentHotkeyLabel.action),
                    true)
            else
...
function removeHotkey()
    ...
    local gameRootPanel = modules.game_interface.getRootPanel()
    configValueChanged = true
    if MouseActions[currentHotkeyLabel.keyCombo] then
        print(currentHotkeyLabel.keyCombo, MouseActions[currentHotkeyLabel.keyCombo])
        --g_mouse.unbindPress(currentHotkeyLabel.keyCombo, MouseActions[currentHotkeyLabel.keyCombo], gameRootPanel)
    else
...
function hotkeyMCapture(assignWindow, pos, button)
    for keyCombo, buttonAction in pairs(MouseActions) do
