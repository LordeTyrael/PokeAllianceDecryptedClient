...
function selectFirstLocale(name)
  ...
  local errorBox = displayErrorBox(tr('Change Language'), tr("Please reopen the client for the changes to take effect."))
  if not EnterGame.changeClientLanguage(name) then
    errorBox:destroy()
  end
  g_settings.save()
end
...
function init()
  ...
  if userLocaleName ~= 'false' and setInitLocale(userLocaleName) then
    pdebug('Using configured locale: ' .. userLocaleName)
  else
    setInitLocale(defaultLocaleName)
    --connect(g_app, { onRun = createWindow })
  end
end

function setInitLocale(name)
