...
local function updateFiles(data, keepCurrentFiles)
  ...
  for file, checksum in pairs(localFiles) do
    ...
    elseif not g_resources.isEncrypted() and string.find(file, "data/things") then
      table.insert(finalFiles, file)
    elseif g_resources.isEncrypted() and g_resources.isLoadedFromArchive() and string.find(file, "data/things") then
      table.insert(finalFiles, file)
    end
  end
  ...
  local toUpdate = {}
  local binary = nil
  if g_resources.isEncrypted() and not g_resources.isLoadedFromArchive() then
  ...
  local forceRestart = g_resources.isEncrypted() and not g_resources.isLoadedFromArchive()
