...
function onGameEditText(id, itemId, maxLength, text, writter, time)
  ...
  local name = text:match('firstElement')
  local type = text:match('info')
  ...
  local closeButton = textWindow:getChildById('closeButton')
  local titleLabel = textWindow:recursiveGetChildById('titleLabel')
  local charCounter = textWindow:getChildById('charCounter')
  ...
  if not writeable then
    titleLabel:setText(tr('Show Text'))
    cancelButton:hide()
    cancelButton:setWidth(0)
    okButton:setMarginRight(0)
  else
    titleLabel:setText(tr('Edit Text'))
  end
  ...
  if writeable then
    local function updateCounter()
      charCounter:setText(string.format('%d/%d', #textEdit:getText(), maxLength))
    end
    charCounter:show()
    updateCounter()
    textEdit.onTextChange = function() updateCounter() end
  end
...
function onGameEditList(id, doorId, text)
  ...
  titleLabel:setText(tr('Edit List'))
