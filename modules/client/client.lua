local musicFilename = "/sounds/ambience/startup"
...
function startup()
  ...
  if musicChannel ~= nil then
    musicChannel:enqueue(musicFilename, 3, 0.03)
  end
  ...
  connect(g_game, { onGameEnd = function()
    if g_sounds ~= nil and musicChannel ~= nil then
      g_sounds.stopAll()
      musicChannel:enqueue(musicFilename, 3, 0.03)
    end
  end })
...
function init()
  ...
  g_window.setIcon('/images/pka/clienticon')
  if g_discord then
    g_discord.init()
    g_discord.update()
  end
