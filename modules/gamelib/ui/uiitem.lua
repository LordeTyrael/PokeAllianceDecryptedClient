...
function UIItem:onDrop(widget, mousePos, forced)
  --local moveWidget = modules.game_market.getMarketOfferWindow()
  --local marketOffer = moveWidget:getChildByPos(mousePos)
  --if marketOffer then
  --    modules.game_market.addMarketOffer(widget)
  --    return true
  --end
    if self:getClassName() == 'UIItem' and self:isForceMove() then
        forced = true
    end
    ...
    if self:getId() == "pokeball" then
        modules.game_stars.dropPokeball(self, item)
        return
    end
...
function UIItem:onMouseRelease(mousePosition, mouseButton)
  ...
  -- Handle searchable items with Shift pressed for magnifying glass functionality
  if self:isSearchable() and g_keyboard.isShiftPressed() and mouseButton == MouseLeftButton then
    local item = self:getItem()
    if item and self:containsPoint(mousePosition) then
      g_game.requestItemDrops(item)
      return true
    end
  end
