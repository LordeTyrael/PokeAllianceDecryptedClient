-- Estado de exp/hora COMPARTILHADO entre game_trainer (escreve, em checkExpSpeed) e game_playeractionbar
-- (lê). PRECISA ser declarado aqui: gamelib NÃO é sandboxed, então isto é um global de verdade,
-- visível nos dois módulos. Declarar em qualquer módulo com `sandboxed: true` cria um global só do
-- sandbox daquele módulo — foi assim que o game_trainer passou a ver nil e estourar em onLevelChange.
...
function g_game.formatDate(dateStr)
  local year, month, day = dateStr:match("(%d+)%-(%d+)%-(%d+)")
  if not year or not month or not day then
    g_logger.error("Invalid date format")
    return
  end
  ...
  -- TODO: getLanguage
  if lang == "pt" then
    return string.format("%d de %s de %d", day, tr(months[month]), year)
  else
    return string.format("%s %d, %s", tr(months[month]), day, year)
  end
end

function g_game.formatShortDate(dateStr)
  local year, month, day = dateStr:match("(%d+)%-(%d+)%-(%d+)")
  if not year or not month or not day then
    g_logger.error("Invalid date format")
    return
...
function g_game.formatStringToTime(dateStr)
    local y, m, d, H, M, S =
        dateStr:match("^(%d+)%-(%d+)%-(%d+)%s+(%d+):(%d+):(%d+)$")
    if not y then
        y, m, d = dateStr:match("^(%d+)%-(%d+)%-(%d+)$")
    end

    if not y then
        g_logger.error("Invalid format: " .. tostring(dateStr))
    end
...
function g_game.getFormatTimeFromMs(milliseconds)
    return formatDuration((tonumber(milliseconds) or 0) / 1000)
end