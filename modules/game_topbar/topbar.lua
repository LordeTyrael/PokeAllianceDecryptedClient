...
function menu(mouseButton)
    ...
    if expPanel:isVisible() then
        menu:addOption(tr("Hide Experience Level"), function() toggleSkillPanel(id) end)
    else
        menu:addOption(tr("Show Experience Level"), function() toggleSkillPanel(id) end)
    end
    for i, child in ipairs(topBar.skills:getChildren()) do
        local id = child:getId()
        if id ~= "stats" then
            if child:isVisible() then
                menu:addOption(tr("Hide %s Level", id), function() toggleSkillPanel(id) end)
            else
                menu:addOption(tr("Show %s Level", id), function() toggleSkillPanel(id) end)
            end
        end
    end
