function OutputMessage:addData(data)
  if type(data) == 'boolean' then
    self:addU16(NetworkMessageTypes.Boolean)
    self:addU8(booleantonumber(data))
  elseif type(data) == 'number' then
    if math.isu8(data) then
      self:addU16(NetworkMessageTypes.U8)
      self:addU8(data)
    elseif math.isu16(data) then
      self:addU16(NetworkMessageTypes.U16)
      self:addU16(data)
    elseif math.isu32(data) then
      self:addU16(NetworkMessageTypes.U32)
      self:addU32(data)
    elseif math.isu64(data) then
      self:addU16(NetworkMessageTypes.U64)
      self:addU64(data)
    else -- negative or non integer numbers
      self:addU16(NetworkMessageTypes.NumberString)
      self:addString(tostring(data))
    end
  elseif type(data) == 'string' then
    self:addU16(NetworkMessageTypes.String)
    self:addString(data)
  elseif type(data) == 'table' then
    self:addU16(NetworkMessageTypes.Table)
    self:addTable(data)
  else
