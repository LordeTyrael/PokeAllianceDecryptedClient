...
-- Legacy key name aliases for backwards compatibility with saved profiles
local keyNameAliases = {
  ['numpad+'] = 'numpadadd',
...
function retranslateKeyComboDesc(keyComboDesc)
  ...
  for i,currentKeyDesc in ipairs(keyComboDesc:split('+')) do
    local trimmed = currentKeyDesc:trim():lower()
    trimmed = keyNameAliases[trimmed] or trimmed
    for keyCode, keyDesc in pairs(KeyCodeDescs) do
      if keyDesc:lower() == trimmed then
...
function g_keyboard.unbindKeyDown(keyComboDesc, arg1, arg2)
  ...
  local translated = retranslateKeyComboDesc(keyComboDesc)
  if translated == nil then return end
  disconnect(widget.boundKeyDownCombos, translated, callback)
end
...
function g_keyboard.unbindKeyUp(keyComboDesc, arg1, arg2)
  ...
  local translated = retranslateKeyComboDesc(keyComboDesc)
  if translated == nil then return end
  disconnect(widget.boundKeyUpCombos, translated, callback)
end
