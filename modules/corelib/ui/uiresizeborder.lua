...
function UIResizeBorder:onMouseMove(mousePos, mouseMoved)
  if self:isPressed() then
    ...
    if self.vertical then
      ...
      if self.inverted then delta = -delta end
      local oldSize = parent:getHeight()
      newSize = math.min(math.max(oldSize + delta, self.minimum), self.maximum)
      parent.userResizing = true
      parent:setHeight(newSize)
      parent.userResizing = nil
      if self.inverted then
        parent:setMarginTop(parent:getMarginTop() - (newSize - oldSize))
      end
    else
      local delta = mousePos.x - self:getX() - self:getWidth()/2
      if self.inverted then delta = -delta end
      local oldSize = parent:getWidth()
      newSize = math.min(math.max(oldSize + delta, self.minimum), self.maximum)
      parent:setWidth(newSize)
      if self.inverted then
        parent:setMarginLeft(parent:getMarginLeft() - (newSize - oldSize))
      end
    end
