...
function UIMoveableTabBar:selectTab(tab)
  ...
  -- A tab still listed in self.tabs can already be detached (mass removal, e.g. game_chat clear()
  -- walking removeTab -> selectPrevTab). Selecting an orphan half-applies state and then crashes on
  -- tab:getParent() below, so bail before touching anything.
  if not tab or tab:isDestroyed() or not tab:getParent() then return end
...
function UIMoveableTabBar:selectPrevTab()
  ...
  if not prevTab or prevTab:isDestroyed() or not prevTab:getParent() then
    return
  end
