...
function UIMiniWindowContainer:fitAll(noRemoveChild)
  ...
  for i=1,#removeChildren do
    ...
    if not (self.overflowHandler and self.overflowHandler(child)) then
...
function UIMiniWindowContainer:onDrop(widget, mousePos)
  if not widget.UIMiniWindowContainer then
    return false
  end

  -- consume so the drop does not fall through to lower-z item slots below the panel
  if widget.floating then
    return true
  end
  ...
  if widget.movedWidget and self:hasChild(widget.movedWidget) then
...
function UIMiniWindowContainer:order()
  ...
  table.sort(children, function(a, b)
    local indexA = a.miniIndex or a.autoOpen or a.index or 999
    local indexB = b.miniIndex or b.autoOpen or a.index or 999
